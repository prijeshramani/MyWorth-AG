...
export class MutualFundValuationStrategy implements IValuationStrategy {
  public readonly assetType = 'MUTUAL_FUND';
  public readonly name = 'Mutual Fund NAV Strategy';
  public readonly valuationMethod = 'MUTUAL_FUND_NAV';
  ...
  public value(context: ValuationContext): ValuationResult {
    ...
    auditTrail.push(`[MutualFundValuation] Valuating ${qty} units of MUTUAL_FUND as of ${context.valuationDate}`);

    if (qty <= 0) {
      warnings.push(`Quantity is ${qty}; NAV valuation is 0`);
      return {
        success: true,
        ...
        dataQuality: 'HIGH',
        priceSource: context.priceSnapshot?.source || 'AMFI',
        warnings,
        errors,
        auditTrail,
        engineVersion: '1.0.0'
    ...
    const priceSource = context.priceSnapshot?.source || 'AMFI';

    if (nav <= 0) {
      warnings.push(`No active NAV available for MUTUAL_FUND ${context.assetId || ''}`);
    }

    let dataQuality: 'HIGH' | 'MEDIUM' | 'LOW' | 'STALE' = 'HIGH';
    if (marketCalendar.isStalePrice(priceDate, context.valuationDate, 5)) {
      dataQuality = 'STALE';
      warnings.push(`NAV date ${priceDate} is stale relative to valuation date ${context.valuationDate}`);
    }
    ...
    auditTrail.push(`[MutualFundValuation] Qty: ${qty}, NAV: ${nav} (${priceSource}), MarketValue: ${marketValue}, CostBasis: ${costBasis}, UnrealizedGain: ${unrealizedGain} (${unrealizedGainPercent}%)`);
...
export const mutualFundValuationStrategy = new MutualFundValuationStrategy();
valuationRegistry.register(mutualFundValuationStrategy);
