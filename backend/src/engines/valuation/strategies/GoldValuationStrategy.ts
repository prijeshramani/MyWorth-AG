...
export class GoldValuationStrategy implements IValuationStrategy {
  public readonly assetType = 'GOLD';
  public readonly name = 'Gold Bullion & SGB Market Price Strategy';
  public readonly valuationMethod = 'PER_GRAM_MARKET_PRICE';
  ...
  public value(context: ValuationContext): ValuationResult {
    ...
    auditTrail.push(`[GoldValuation] Valuating ${qty} grams/units of GOLD as of ${context.valuationDate}`);

    if (qty <= 0) {
      warnings.push(`Quantity is ${qty}; Gold valuation is 0`);
      return {
        success: true,
        ...
        dataQuality: 'HIGH',
        priceSource: context.priceSnapshot?.source || 'BULLION_EXCHANGE',
        warnings,
        errors,
        auditTrail,
        engineVersion: '1.0.0'
    ...
    const pricePerGram = context.priceSnapshot?.value || 0;
    const priceDate = context.priceSnapshot?.timestamp || context.valuationDate;
    const priceSource = context.priceSnapshot?.source || 'BULLION_EXCHANGE';

    if (pricePerGram <= 0) {
      warnings.push(`No active gold market price available for GOLD ${context.assetId || ''}`);
    }
    ...
    const marketValue = CurrencyPrecision.roundMoney(qty * pricePerGram);
    ...
    auditTrail.push(`[GoldValuation] Qty: ${qty} grams, Price/Gram: ${pricePerGram} (${priceSource}), MarketValue: ${marketValue}, CostBasis: ${costBasis}, UnrealizedGain: ${unrealizedGain} (${unrealizedGainPercent}%)`);
...
export const goldValuationStrategy = new GoldValuationStrategy();
valuationRegistry.register(goldValuationStrategy);
